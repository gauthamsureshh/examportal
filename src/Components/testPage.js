...
function TestPage() {
    ...
    const questions = question.filter(q => q.category === category); // fetches the question according to the exam category selected by the user.
    const [timeLeft, setTimeLeft] = useState(300); // 5 minute timer
    const [isModalOpen, setIsModalOpen] = useState(false);

    // If user is not found, the test-page will redirect to login page
    useEffect(() => {
        if (user === 0) {
            navigate('/');
        }
    ...
    // Function to count the number of correct answers among all the selected options.
    ...
    // Function to count the number of missed questions
    const countMissedQuestions = useCallback(() => {
        let count = 0;
    ...
    // Memoized handleSubmission function
    const handleSubmission = useCallback(() => {
        ...
        navigate('/resultpage');
    }, [countCorrectAnswer, countMissedQuestions, dispatch, navigate, notes, questions, timeLeft]);
